# Federated Witness Manifest Spec
...
Define how Registry Witness nodes federate with each other while using
...
- `registry-witness` performs peer authentication, authorization, claim
...
- `registry-relay` may publish or aggregate manifests, but does not own Witness
...
flowchart LR
  ...
  Witness["registry-witness\nruntime enforcement"]
  Platform["registry-platform\nshared primitives"]
  Relay["registry-relay\noptional publisher"]

  Manifest -->|"declares capabilities"| Witness
  Platform -->|"crypto, auth, audit, HTTP"| Witness
  Relay -.->|"hosts or aggregates metadata"| Manifest
  Witness -->|"signed facts and credentials"| Witness
```

## Background

Registry Witness is designed to evaluate configured claims against authorized
...
serve as the control-plane artifact for a federated Witness network.

The missing pieces are:

1. Manifest fields that describe a Witness node, its federation endpoint,
...
- Local policy wins. A remote manifest can advertise capability, but the local
  Witness decides whether to call it or accept results from it.
- Sensitive configuration stays out of Manifest. API keys, bearer tokens,
  ...
  Witness config or secret stores.
- Audit sharing is checkpoint-based by default. Nodes publish signed chain heads,
  not raw audit events.

## V1 Protocol Decisions

These decisions are normative for `registry-witness-federation/v0.1` and remove
ambiguity from later stages:

- V1 defines Registry Witness JWT profiles. It does not claim that the signed
  federation document is an OpenID Federation entity statement.
- V1 signed metadata uses `typ = registry-witness-federation-metadata+jwt`.
- V1 signed peer requests use `typ = registry-witness-request+jwt`.
- V1 signed peer responses use `typ = registry-witness-response+jwt`.
- V1 signed audit checkpoints use `typ = registry-witness-audit-checkpoint+jwt`.
...
- `/.well-known/registry-witness` is an unregistered private bootstrap pointer
  in V1. Registration can be pursued later if the protocol stabilizes.
- `application/dc+sd-jwt` is the credential media type used by current Witness
...
| --- | --- |
| Publishing Witness | Publishes a Registry Manifest federation profile and serves federation APIs |
| Consuming Witness | Discovers peer metadata, validates trust, requests evaluations or credentials |
...
federation:
  ...
  discovery_url: https://social-protection.example.gov/.well-known/registry-witness
  protocol_versions:
...
Federated Witness should use that existing shape with a new `kind` value:
...
`access.kind = registry-witness` means the offering is served through a
Registry Witness federation endpoint. It does not mean every peer may call it.
Runtime access is still controlled by local Witness peer policy.

For `access.kind = registry-witness`, `ruleset` is a public stable identifier
for the Witness runtime rules used to produce the advertised output. It is the
...
3. A peer request selects a profile id. The serving Witness resolves that
   profile to its `ruleset` and checks that the selected evidence offering also
   advertises the same `ruleset`.
4. Local Witness config maps the public `ruleset` id to private claim
...
```text
registry-witness-federation/v<major>.<minor>
experimental:registry-witness-federation/v<major>.<minor>/<label>
```

Unknown non-experimental values are validation errors. Experimental values may
be rendered, but consuming Witness nodes must deny them unless local policy
explicitly opts in.

## Credential And Evaluation Profiles

Registry Manifest should add a profile declaration for the outputs a Witness can
...
evaluation_profiles:
  - id: disability_status_predicate
    ...
    lookup_keys:
      - national_id
    output_formats:
      - application/vnd.registry-witness.evaluation+json
    disclosure:
      modes:
        - predicate
```
...
flowchart TD
  ...
  Ruleset["Ruleset"]
  Runtime["Private Witness runtime config"]
...
trust:
  ...
  accepted_statement_types:
    - registry-witness-trust-bundle+jwt
```

This is not an allowlist. It only tells consumers which public trust framework
the node claims to participate in. A consuming Witness must still validate the
trust chain and local policy.

V1 should support static local trust first, then
`registry-witness-trust-bundle+jwt`. OpenID Federation-style entity statements
...
`/.well-known/registry-witness` should be a small discovery document that points
...
```json
{
  ...
  "protocol_versions": ["registry-witness-federation/v0.1"]
}
```

The signed federation document should include:

- protected header `typ = registry-witness-federation-metadata+jwt`.
...
Witness or publisher. `registry-manifest-core` must not load private keys or sign
documents. Signing belongs in the CLI, publisher, or Witness runtime, using
...
- `access.kind = registry-witness` requires `conforms_to`, `endpoint_url`,
...
- `credential_profiles[*].ruleset` and `evaluation_profiles[*].ruleset` must
  not be empty.
- `access.ruleset` for `registry-witness` offerings must resolve to at least
...
required only for `kind = registry-witness`, that is a schema migration and must
check existing non-Witness fixtures before changing serde behavior.

## Witness Runtime Config

Witness should add an optional `federation` runtime config block. It is disabled
by default.
...
federation:
  ...
  signing:
    federation_document_key_id: witness-fed-2026-05
    federation_document_key_env: REGISTRY_WITNESS_FEDERATION_JWK
    request_signing_key_id: witness-request-2026-05
    request_signing_key_env: REGISTRY_WITNESS_FEDERATION_REQUEST_JWK
  metadata:
    manifest_path: /etc/registry-witness/metadata.yaml
  ...
  replay:
    store: redis
    url_env: REGISTRY_WITNESS_REPLAY_REDIS_URL
...
The replay store must be shared by all active Witness instances in a deployment.
...
Witness must reject signed requests where `exp - iat` exceeds
...
live behind a small Witness replay abstraction first. Promote it to a shared
...
Witness instance. It is not a network-wide revocation channel; coordinated
...
```text
GET /.well-known/registry-witness
  ...
  "evaluation_profiles": [
    {
      "id": "disability_status_predicate",
      "ruleset": "disability-status-v1",
      "output_formats": ["application/vnd.registry-witness.evaluation+json"]
    }
  ],
  "credential_profiles": []
}
...
`registry-witness-trust-bundle+jwt` or a JSON collection of such JWTs:
...
| Case | Status | `type` |
| --- | --- | --- |
| malformed JSON or JWT | `400` | `https://registry-witness.dev/problems/federation/bad-request` |
| missing or invalid authentication | `401` | `https://registry-witness.dev/problems/federation/unauthorized` |
| known peer denied by local policy | `403` | `https://registry-witness.dev/problems/federation/forbidden` |
| unsupported protocol or profile | `422` | `https://registry-witness.dev/problems/federation/unsupported` |
| replayed `jti` or nonce | `409` | `https://registry-witness.dev/problems/federation/replay` |
| rate limit | `429` | `https://registry-witness.dev/problems/federation/rate-limited` |
| source or issuer unavailable | `503` | `https://registry-witness.dev/problems/federation/temporarily-unavailable` |

Example:

```json
{
  "type": "https://registry-witness.dev/problems/federation/forbidden",
...
Witness may add a constant-time response floor for denial classes that could
...
- `nonce` is issued by the serving Witness when a flow requires a freshness
...
replacement for signed request objects. Witness must verify the durable object
...
The serving Witness must reject the request when `subjects.len()` exceeds local
...
```json
{
  "results": [
    ...
    },
    {
      ...
      "ok": false,
      "error": {
        "type": "https://registry-witness.dev/problems/federation/temporarily-unavailable",
...
V1 does not allow a consuming Witness to act as a trust proxy for a wallet
holder proof. Holder proof verification is end-to-end with the issuing Witness.
The consuming Witness has two allowed roles:

1. Discovery and handoff: the consuming Witness discovers the issuing Witness
   and returns or forwards a credential offer so the holder can call the issuing
   Witness directly.
2. Transparent byte relay: the consuming Witness forwards the holder's request bytes
   without modifying the proof, nonce, holder key, or audience. The issuing
   Witness remains responsible for nonce issuance and proof verification.

The consuming Witness must not substitute a holder key, mint an issuer nonce,
...
Witness, not the consuming Witness. Verifier trust configuration must therefore
include all issuing Witness nodes whose credentials may be presented.

Transparent byte relay creates a metadata-collection surface: the relaying
Witness can observe which holder proof identifiers request which credential
...
Status policy defines how the consuming Witness treats credential status and
...
Credential issuance must preserve current Witness invariants:
...
signed metadata. Operators may pin a lower version per peer. The serving Witness
...
Add manifest fixtures that describe a federated Witness node and a consuming
Witness fixture that references it.

Definition of Done:

- `registry-manifest` has a sample federated Witness manifest fixture.
- `registry-witness` has a sample federation runtime config.
- fixture docs identify what is public metadata versus local runtime policy.
- fixture docs state that signing happens in CLI, publisher, or Witness runtime,
...
### Stage 3: Witness Peer Discovery And Trust Cache

Registry Witness can fetch and validate a peer discovery document and signed
...
with a Witness-local replay abstraction; extract a `registry-platform-replay`
...
- two Witness instances sharing the store reject replay across instances;
- idempotent retry returns the prior response only for byte-identical payloads.

### Stage 4: Delegated Evaluation

Registry Witness can accept and send delegated evaluation requests.
...
Witness should emit metrics for:
...
Existing Witness APIs must continue to work when federation is disabled.
Existing Registry Manifest files must continue to validate when the new
federation fields are absent.

`access.kind` remains open to non-Witness values. Existing `evidence-server`,
...
1. Registry Manifest renders unsigned federation metadata for one Witness node.
2. Registry Witness configures a static peer allowlist.
3. Registry Witness accepts a signed single-subject delegated evaluation request
