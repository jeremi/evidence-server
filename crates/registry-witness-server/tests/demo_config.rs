...
use registry_witness_core::{BulkMode, StandaloneRegistryWitnessConfig};
...
fn openspp_disability_demo_config_loads_validates_and_builds_router() {
    ...
    let config_path = manifest_dir
        ...
        .and_then(std::path::Path::parent)
        .expect("workspace root")
        .join("demo/config/openspp-disability-registry-witness.yaml");
    ...
    config.audit.path = Some(
        temp.path()
            .join("openspp-disability-registry-witness-audit.jsonl")
            .to_string_lossy()
            .into_owned(),
    );
